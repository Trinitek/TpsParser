...
namespace TpsParser.Tests.Tps.KeyRecovery
{
    ...
    public class RecoveryStateTest
    {
        private (Block plaintext, Block encrypted, IEnumerable<Block> blocks) BuildBlocks()
        {
            ...
            k.Encrypt64(new TpsReader(cb0));
            k.Encrypt64(new TpsReader(cseq));

            var plaintext = new Block(new TpsReader(plain), isEncrypted: false);
            var encrypted = new Block(new TpsReader(crypt), isEncrypted: true);

            var cryptSeq = new Block(new TpsReader(cseq), isEncrypted: false);
            var cryptB0 = new Block(new TpsReader(cb0), isEncrypted: false);
