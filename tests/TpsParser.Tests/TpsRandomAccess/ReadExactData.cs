...
internal sealed class ReadExactData
{
    ...
    [TestCase((ushort)0x0102, new byte[] { 0x01, 0x02 })]
    [TestCase(unchecked((ushort)0x8182), new byte[] { 0x81, 0x82 })]
    public void ShouldReadUnsignedShortBE(ushort value, byte[] data)
    {
        var rx = new TpsRandomAccess(data, Encoding.ASCII);
        ushort parsed = rx.ReadUnsignedShortBE();
    ...
    [TestCase((ushort)0x0102, new byte[] { 0x02, 0x01 })]
    [TestCase(unchecked((ushort)0x8182), new byte[] { 0x82, 0x81 })]
    public void ShouldReadUnsignedShortLE(ushort value, byte[] data)
    {
        var rx = new TpsRandomAccess(data, Encoding.ASCII);
        ushort parsed = rx.ReadUnsignedShortLE();
    ...
    [TestCase(unchecked((uint)0x01020304), new byte[] { 0x01, 0x02, 0x03, 0x04 })]
    [TestCase(unchecked((uint)0x81828384), new byte[] { 0x81, 0x82, 0x83, 0x84 })]
    public void ShouldReadUnsignedLongBE(uint value, byte[] data)
    {
        var rx = new TpsRandomAccess(data, Encoding.ASCII);
        var parsed = rx.ReadUnsignedLongBE();
    ...
    [TestCase(unchecked((uint)0x01020304), new byte[] { 0x04, 0x03, 0x02, 0x01 })]
    [TestCase(unchecked((uint)0x81828384), new byte[] { 0x84, 0x83, 0x82, 0x81 })]
    public void ShouldReadUnsignedLongLE(uint value, byte[] data)
    {
        var rx = new TpsRandomAccess(data, Encoding.ASCII);
        var parsed = rx.ReadUnsignedLongLE();
    ...
    [Test]
    public void ShouldReadZeroTerminatedString_Ascii_OverrideWith_CP850()
    {
        var codepage850 = CodePagesEncodingProvider.Instance.GetEncoding(850);
        var rx = new TpsRandomAccess([0x82, 0x21, 0x22, 0x00, 0x23], Encoding.ASCII);
        var parsed = rx.ReadZeroTerminatedString(codepage850);
    ...
    [Test]
    public void ShouldReadPascalString_Ascii()
    {
        var rx = new TpsRandomAccess([0x02, 0x20, 0x21], Encoding.ASCII);
        var parsed = rx.ReadPascalString();

        using (Assert.EnterMultipleScope())
        {
            Assert.That(parsed, Is.EqualTo(" !"));
            Assert.That(rx.Position, Is.EqualTo(3));
            Assert.That(rx.IsAtEnd);
        }
    }
    ...
    [Test]
    public void ShouldReadPascalString_Ascii_OverrideWith_CP850()
    {
        var codepage850 = CodePagesEncodingProvider.Instance.GetEncoding(850);
        var rx = new TpsRandomAccess([0x02, 0x82, 0x21], Encoding.ASCII);
        var parsed = rx.ReadPascalString(codepage850);
